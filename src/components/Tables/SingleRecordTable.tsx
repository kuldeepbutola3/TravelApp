...
export const toCellInfo = <
  T extends {[key: string]: DataValue},
...
export const SingleRecordTable: React.FC<SingleRecordTableProps> = ({
  cells,
  style,
}) => {
  return (
    <View style={flatten(styles.container, style)}>
      {cells.map((cell) => (
        <View
          key={`single.rowDataContainer.${cell.heading}`}
          style={[styles.row]}>
          <TableHeading
            key={`single.heading.${cell.heading}`}
            horizontalAlignment={'right'}>
            {cell.heading}
          </TableHeading>
          <TableData key={`single.rowData.${cell.heading}.${cell.value}`}>
            {cell.value}
          </TableData>
        </View>
      ))}
